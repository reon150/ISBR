...
describe('Business Exceptions', () => {
  ...
  describe('NotFoundException', () => {
    ...
    it('should create not found exception with custom code and details', () => {
      ...
      expect(exception.message).toBe("Inventory with identifier '456' not found");
      expect(exception.code).toBe(ErrorCode.INVENTORY_NOT_FOUND);
      expect(exception.statusCode).toBe(404);
  ...
  describe('AlreadyExistsException', () => {
    it('should create already exists exception', () => {
      const exception = new AlreadyExistsException('Resource', '123');

      expect(exception.message).toBe("Resource with identifier '123' already exists");
    ...
    it('should create already exists exception with custom code and details', () => {
      ...
      expect(exception.message).toBe("Inventory with identifier 'SKU123' already exists");
      expect(exception.code).toBe(ErrorCode.RESOURCE_ALREADY_EXISTS);
      expect(exception.statusCode).toBe(409);
      expect(exception.details).toEqual({
        resource: 'Inventory',
  ...
  describe('BusinessRuleViolationException', () => {
    it('should create business rule violation exception', () => {
      const exception = new BusinessRuleViolationException('Cannot delete active inventory');

      expect(exception.message).toBe('Business rule violation: Cannot delete active inventory');
      expect(exception.code).toBe(ErrorCode.BUSINESS_RULE_VIOLATION);
      expect(exception.statusCode).toBe(422);
      expect(exception.details).toEqual({
        rule: 'Cannot delete active inventory',
      });
    });
