...
async function bootstrap(): Promise<void> {
  ...
  const config: Omit<OpenAPIObject, 'paths'> = new DocumentBuilder()
    ...
    .setDescription(
      'API documentation for the Inventory microservice - Manages inventory levels and stock movements',
    )
    .setVersion('1.0')
    .addTag('inventory', 'Inventory management endpoints')
    .addTag('movements', 'Stock movement tracking')
    .addBearerAuth(
      {
        type: 'http',
        scheme: 'bearer',
  ...
  logger.log(`Inventory Service is running on: http://localhost:${port}/api`);
